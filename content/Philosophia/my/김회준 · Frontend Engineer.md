...
[텔코](https://www.tel-co.net/)는 여행지·맛집·카페를 탐색하고 여행 플랜을 만들어 공유하는 서비스입니다. 여행 플랜을 중심으로 웹 프론트엔드 기능과 구조 개선을 맡았습니다.

- 여행 플랜 생성부터 편집과 공유까지 주요 흐름을 개발했습니다. 일정별 장소 구성, 드래그 앤 드롭 정렬, 날짜 이동·삭제, 과거 플랜 편집, 다른 플랜 저장과 공유용 스냅샷을 구현했습니다.
- 플랜·날짜·장소의 관계를 별도 모듈로 분리하고 React Query의 query·mutation에 연결했습니다. 백엔드 브랜치의 OpenAPI 문서에서 타입과 API 클라이언트를 다시 생성할 수 있도록 스크립트도 정리했습니다.
- 네이버 지도와 장소 검색을 플랜 편집에 연결했습니다. 선택한 일정과 지도 마커·이동 범위를 동기화하고, 검색·위시리스트·주변 장소에서 플랜에 장소를 추가하는 흐름을 만들었습니다.
- 카페·맛집·여행에 나뉘어 있던 스팟 상세 화면을 공통 진입점과 콘텐츠 구조로 정리했습니다. 검색이나 플랜 편집의 맥락을 유지한 채 상세를 볼 수 있도록 경로 기반 바텀시트를 적용했습니다.
- 모바일 앱 WebView에서 라우팅 이력, iOS Safari의 safe area와 모달 스크롤 잠금 문제를 해결했습니다. 메인 데이터의 서버 프리패치, 스크롤 처리와 지도 마커 렌더링도 개선했습니다.
