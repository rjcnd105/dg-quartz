...
- **암시적 지식을 명시적 도메인으로.** 암묵지를 코드·문서·타입·스키마·상태 모델로 옮겨 팀, 회사 차원의 명시적 도메인으로 만듭니다. 이러한 암묵지는 눈에 쉽게 보이지 않는 생산성 저하가 굉장히 크다고 생각합니다.
- **구조적 설계 기반의 빠른 프로덕트 이터레이션.** 초기에 도메인·경계·데이터 레이어를 잡고, 그 위에서 피처와 피드백 반영 속도를 끌어올립니다. 추상화는 프로젝트 단계에 맞춰 점진적으로 확장합니다.
