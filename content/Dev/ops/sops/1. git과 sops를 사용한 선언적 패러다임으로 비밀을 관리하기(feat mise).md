---
{"publish":true,"created":"2025-09-22T03:07:38Z","modified":"2025-11-19T01:49:36Z","tags":["sops","mise","age","gitops"],"cssclasses":""}
---
...
만약 비밀번호 파일 자체를 암호화해서 권한 명시와 함께 git에 커밋할 수 있다면 어떨까요?
...
파일 형식도 dotenv(.env), yaml, json, toml까지 폭 넓게 지원합니다.
이 글에서는 간단히 현대화 암호 툴인 age를 사용하겠습니다.

팀원들과 개발 환경을 손쉽게 공유하고 사용할 수 있는 [mise](https://mise.jdx.dev/)를 통해 sops와 age를 설치하고 사용하는 예제를 보여드리겠습니다.
(mise를 사용하지 않을 것이라면 sops, age를 brew로 설치하시고 mise 관련 내용은 스킵하시면 됩니다.)

***

## 시작하기

### 1. [mise](https://mise.jdx.dev/)를 설치하고 내 프로젝트 루트에서 [activate](https://mise.jdx.dev/getting-started.html#activate-mise) 시킵니다.
...
path_regex에 해당 유저가 명시되어 있어야 해당 경로의 비밀에 접근할  수 있습니다.

저는 암호화된 비밀을 구분하기 위해 파일명에 .enc를 붙힙니다.

.sops.yaml은 [code owner](https://docs.github.com/ko/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners) 설정을 통해 변경시 관리자가 검토할 수 있게 해야합니다.
...
run = '''
	sops exec-env ../secrets/shared/secrets.enc.yaml '
...
**use case 2)**
기존의 env 파일을 암호화하고 싶을 때도 있습니다.
하지만 `cat ./app.env | sops encrypt`처럼 파이프를 사용하면, sops는 어떤 암호화 규칙(`creation_rules`)을 적용해야 할지 알 수 없습니다.
...
이미 dotenv로 암호화되어 있던 파일 형식만 yaml로 변경하기
```bash
sops -d ./app.enc.env | sops encrypt --filename-override ./app.enc.env --output ./app.enc.yaml --output-type yaml
```
...
이제 sops를 통해 비밀과 비밀에 대한 권한을 명시적으로 git을 통해 형상 관리를 할 수 있습니다.

**keygroup을** 통해 그룹을 설정하고 **role**을 통해 역할 분리와 특정 **context** 내의 비밀만 접근할 수 있게 하는 등 advanced한 사용 사례에 대해서도 구현되어 있으니 더 자세한 sops의 사용법은 [github sops readme](https://github.com/getsops/sops?tab=readme-ov-file#sops-secrets-operations)에서 읽으시면 됩니다.
...
[[Dev/ops/sops/draft - 2. sops를 docker 내에서 사용하기]]
